...
class BillingService:
    """Transactional plans, subscriptions, entitlements, quotas, and usage metering.

    Monetary amounts are always integer minor units. Usage events are immutable and
    idempotent per organization and external event key.
    """
    ...
    def create_plan(
        ...
        name: str,
        currency: str,
        recurring_amount_minor: int | str | Decimal,
        ...
        with self._connection(immediate=True) as db:
            db.execute(
                ...
                (
                    ...
                    name.strip(),
                    normalized_currency,
                    _minor_units(recurring_amount_minor),
                    billing_interval,
                    "active",
                    "{}",
                    now,
            ...
            for key, definition in sorted(entitlements.items()):
                ...
                db.execute(
                    ...
                    (plan_id, key.strip(), int(bool(definition.get("enabled", True))), quota, period),
                )
    ...
    def get_plan(self, plan_id: str) -> dict[str, Any]:
        with self._connection() as db:
            ...
            rows = db.execute(
                "SELECT * FROM plan_entitlements WHERE plan_id=? ORDER BY entitlement_key", (plan_id,)
            ).fetchall()
        result = dict(plan)
        result["metadata"] = json.loads(result.pop("metadata_json"))
        result["entitlements"] = [dict(row) for row in rows]
        return result
    ...
    def set_override(
        ...
        with self._connection(immediate=True) as db:
            db.execute(
                ...
                (organization_id, entitlement_key, int(enabled), quota_limit, expires_at, reason.strip(), _utcnow()),
            )
        ...
        with self._connection() as db:
            override = db.execute(
                """SELECT * FROM entitlement_overrides
                   WHERE organization_id=? AND entitlement_key=?
            ...
            row = db.execute(
                """SELECT pe.* FROM subscriptions s
                   ...
                   WHERE s.organization_id=? AND s.status IN ('trialing','active')
                     ...
                     AND pe.entitlement_key=? LIMIT 1""",
                (organization_id, moment, moment, entitlement_key),
            ).fetchone()
        if row is None:
            return {"enabled": 0, "quota_limit": None, "quota_period": None, "source": "none"}
        return dict(row) | {"source": "plan"}
    ...
    def record_usage(
        ...
        meter_key: str,
        quantity: int = 1,
        amount_minor: int | str | Decimal = 0,
        ...
        with self._connection(immediate=True) as db:
            existing = db.execute(
                "SELECT * FROM usage_events WHERE organization_id=? AND event_key=?",
                (organization_id, event_key),
            ).fetchone()
            if existing is not None:
                return dict(existing) | {"metadata": json.loads(existing["metadata_json"]), "idempotent": True}
            subscription = db.execute(
                """SELECT id FROM subscriptions WHERE organization_id=?
                   AND status IN ('trialing','active') AND period_start<=? AND period_end>?
                   LIMIT 1""",
        ...
        return dict(row) | {"metadata": json.loads(row["metadata_json"]), "idempotent": False}
    ...
    def require_entitlement(
        ...
        with self._connection() as db:
            used = db.execute(
                """SELECT COALESCE(SUM(quantity),0) used FROM usage_events
                   WHERE organization_id=? AND meter_key=? AND occurred_at>=? AND occurred_at<?""",
                (organization_id, meter_key, period_start, period_end),
            ).fetchone()["used"]
        if used + requested_quantity > quota:
            raise QuotaExceeded(f"quota exceeded for {entitlement_key}")
        return resolved | {"used": used, "remaining": quota - used}
